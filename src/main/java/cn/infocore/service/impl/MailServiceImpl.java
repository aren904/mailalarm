...
public class MailServiceImpl implements MailService {
	...
	public void notifyCenter(Data_ark data_ark, List<Client_> clientList, List<Vcenter> vcList,
			List<Virtual_machine> vmList,List<RDS> rdsList, List<RDSInstance> rdsInstances, Fault... list_fault) {
			...
			try {
				...
				if (fault.getType() == ClientType.SINGLE_VALUE) {
					...
					if (fault.getClient_type() == ClientType.SINGLE_VALUE) {
						...
						for (RDS rds : rdsList) {
							if (fault.getData_ark_id().equals(rds.getDataArkId()) && fault.getClient_id().equals(rds.getRdsId())) {
						...
						for (RDSInstance object : rdsInstances) {
							if (fault.getData_ark_id().equals(object.getDataArkId()) && fault.getClient_id().equals(object.getId())) {
